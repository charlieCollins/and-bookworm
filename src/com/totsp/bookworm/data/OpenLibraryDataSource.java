...
public class OpenLibraryDataSource implements BookDataSource {
	...
	public Book getBook(String isbn) {
		String url = OL_ISBN_SEARCH_PREFIX+isbn;
	...
	private ArrayList<Book> parseResponse(String response, String isbn) throws JSONException {
		...
		if ((jsonString.length() == 2)) {
			Log.w(Constants.LOG_TAG, "HTTP request returned no data (null) - " + OL_ISBN_SEARCH_PREFIX+isbn);
			return null;
		}
		...
		if(jsonBook.getJSONObject("details").has("authors")) {
			book.authors.add(new Author(jsonBook.getJSONObject("details").getJSONArray("authors").getJSONObject(0).getString("name")));
		}
	...
	public ArrayList<Book> getBooks(String searchTerm, int startIndex) {
		if(startIndex == 0)
		{
			String url = OL_BOOK_SEARCH_PREFIX;
			try {
	...
	private ArrayList<Book> getBookDetails(String jsonString) throws JSONException {
		...
		for (int i = 0; i < resultArray.length(); i++) {
			String url = OL_BOOK_DATA_PREFIX+resultArray.getString(i);
			String response = httpHelper.performGet(url, null, null, null);
			...
			if(jsonBook.getJSONObject("result").has("authors")) {
				String url2 = OL_BOOK_DATA_PREFIX+jsonBook.getJSONObject("result").getJSONArray("authors").getJSONObject(0).getString("key");
